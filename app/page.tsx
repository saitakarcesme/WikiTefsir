...
export default async function Home() {
  ...
  return <main><SiteHeader />
    ...
    <footer className="reader-footer"><span>IslamWiki</span><p>{tr ? 'Bağımsız ve kaynak öncelikli bir bilgi projesi.' : 'An independent source-first knowledge project.'}</p><nav><Link href="/surahs">{tr ? 'Kur’an' : 'Quran'}</Link><Link href="/hadith">{tr ? 'Hadis' : 'Hadith'}</Link><Link href="/people">{tr ? 'Kişiler' : 'People'}</Link><Link href="/graph">{tr ? 'Grafik' : 'Graph'}</Link></nav></footer>
  </main>;
}

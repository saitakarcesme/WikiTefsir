...
export function StoryArtwork({ image, title, source, arabic, translation, locale, priority = false }: {
  image: string;
  title: string;
  source: string;
  arabic: string;
  ...
  return <figure className={`story-artwork${flipped ? ' flipped' : ''}`}>
    <button type="button" onClick={() => setFlipped((value) => !value)} aria-pressed={flipped} aria-label={flipped ? (tr ? 'Görseli göster' : 'Show artwork') : (tr ? 'Ayeti göster' : 'Show verse')}>
      <span className="story-artwork-inner">
        <span className="story-artwork-face story-artwork-front"><Image src={image} alt="" fill unoptimized priority={priority} sizes="(max-width: 760px) 100vw, 850px" /></span>

...
export default async function PersonPage({ params }: PersonPageProps) {
  ...
  return <main><SiteHeader /><div className="person-reader-layout">
    ...
    <article className="reader-article">
      ...
      <header className="reader-article-header"><span className="reader-overline">{person.kind} in the Quran</span><h1>{person.name}</h1><p className="reader-arabic-title" lang="ar" dir="rtl">{person.arabic}</p><p className="reader-deck">{person.introduction}</p><div className="reader-metadata"><span>{person.role}</span><span>{person.narrative.length} story chapters</span><span>{person.keyReferences.length} key references</span></div><p className="quran-naming-note">{person.quranNaming}</p></header>
      <PersonRelationDiagram person={person} />
      <section className="quran-timeline person-story-flow" aria-labelledby="timeline-title"><div className="reader-section-heading"><span>Across the surahs</span><h2 id="timeline-title">The story through its verses</h2><p>Read continuously: every passage is already open and arranged in narrative order.</p></div>
        {person.narrative.map((stage, stageIndex) => <article id={stage.id} key={stage.id} className="timeline-stage">
          ...
          <div className="timeline-content"><h3>{stage.title}</h3><p className="timeline-summary">{stage.summary}</p>
            <div className="timeline-references">{stage.references.map((reference) => {
              ...
              return <article className="story-verse" key={`${reference.surah}:${reference.ayah}`}><header><Link href={`${getSurahHref(surah)}#verse-${reference.ayah}`}>{surah.nameTransliterated} {reference.surah}:{reference.ayah}</Link></header><div className="timeline-verse"><p lang="ar" dir="rtl">{verse.text}</p><p>{meaning.text}</p><div><SourceDrawer label="Open source" title={`${surah.nameTransliterated} ${reference.surah}:${reference.ayah}`} pdfUrl={source?.pdfUrl} page={source?.page} sourceUrl="https://quranenc.com/en/browse/english_rwwad" sourceLabel="QuranEnc" /></div></div></article>;
            })}</div>
          </div>
        </article>)}
      </section>
      <p className="person-closing-note">{person.closingNote}</p>

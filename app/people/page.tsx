...
export default async function PeoplePage() {
  ...
  return (
    <main><SiteHeader /><div className="reader-index-page">
      ...
      <header className="reader-page-header"><span className="reader-overline">{tr ? 'Bağlantılı makaleler' : 'Connected articles'}</span><h1>{tr ? 'Kur’an’daki kişiler ve topluluklar' : 'People and figures in the Quran'}</h1><p>{tr ? 'Kur’an’da adı geçen bütün peygamberleri anlatı sırasıyla okuyun; ardından hükümdarları, aileleri, melekleri, karşıtları, toplulukları ve adı verilmeyen kişileri keşfedin. Her aşama doğrudan ilgili ayetlere bağlanır.' : 'Read about every prophet named in the Quran in narrative order, then explore rulers, families, angels, adversaries, communities, and unnamed figures. Every stage links directly to the relevant verses.'}</p><div className="people-counts"><a href="#prophets"><strong>25</strong><span>{tr ? 'adı geçen peygamber' : 'named prophets'}</span></a><a href="#figures"><strong>{figures.length}</strong><span>{tr ? 'diğer kişi ve topluluk' : 'other figures'}</span></a></div></header>
      <section id="prophets" className="people-directory-section"><div className="reader-section-heading"><span>{tr ? 'Adı geçen peygamberlerin tamamı' : 'Complete set of named prophets'}</span><h2>{tr ? 'Kur’an’daki peygamberler' : 'Prophets in the Quran'}</h2></div><div className="reader-card-list" aria-label={tr ? 'Peygamber makaleleri' : 'Prophet articles'}>{cards(prophets)}</div></section>

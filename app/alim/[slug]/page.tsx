        ...
        <div className="profile-grid">
          <article className="profile-main">
            <section>
              ...
              {scholar.bookViewerUrl ? <div className="scholar-book-action"><SourceDrawer label={tr ? `${scholar.work} eserini aç` : `Open ${scholar.work}`} title={`${scholar.name} — ${scholar.work}`} viewerUrl={scholar.bookViewerUrl} sourceUrl={scholar.bookSourceUrl} sourceLabel="Internet Archive" /></div> : null}
            ...
            {scholar.linkedCorpus ? <section>
              ...
              <div className="linked-record"><span>Al-Fatihah 1:1</span><strong>{tr ? 'Kaynaklı Arapça tefsir kaydı' : 'Sourced Arabic tafsir record'}</strong><Link href="/surah/fatiha#verse-1">{tr ? 'Kaydı aç' : 'Open record'} →</Link></div>
              <div className="linked-record"><span>Al-Baqarah 2:255</span><strong>{tr ? 'Âyetü’l-Kürsî tefsir kaydı' : 'Ayat al-Kursi tafsir record'}</strong><Link href="/surah/al-baqara#verse-255">{tr ? 'Kaydı aç' : 'Open record'} →</Link></div>
            </section> : null}
          </article>

          <aside className="profile-facts">
            <h2>{tr ? 'Bir bakışta' : 'At a glance'}</h2>
            <dl><div><dt>{tr ? 'Dönem' : 'Period'}</dt><dd>{tr ? scholar.datesTr : scholar.dates}</dd></div><div><dt>{tr ? 'Coğrafya' : 'Geography'}</dt><dd>{tr ? scholar.placeTr : scholar.place}</dd></div><div><dt>{tr ? 'Alanlar' : 'Fields'}</dt><dd>{tr ? scholar.fieldTr : scholar.field}</dd></div><div><dt>{tr ? 'Eser' : 'Work'}</dt><dd>{scholar.work}</dd></div></dl>
